    ...
    public void execute() {
        ...
        if (user != null && password != null) {
            if (log.isDebugEnabled())
                log.debug("Setting HTTP Basic auth for endpoint {} with username {}",getEndpoint(), user);
            client.addFilter(new HTTPBasicAuthFilter(user, password));
        }
        String reqStr = getUpdateRequest().toString();

        if (log.isDebugEnabled())
            log.debug("Sending SPARQL request {} to endpoint {}", reqStr, getEndpoint());
        ClientResponse response = wr.type(WebContent.contentTypeSPARQLUpdate)
        .accept(WebContent.contentTypeResultsXML).post( ClientResponse.class, reqStr);
